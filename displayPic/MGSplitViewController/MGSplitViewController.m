...
{
	return _viewControllers;//[[_viewControllers copy] autorelease];
}
...
- (UIViewController *)masterViewController
{
	if (_viewControllers && [_viewControllers count] > 0) {
		...
		if ([controller isKindOfClass:[UIViewController class]]) {
			return controller;//[[controller retain] autorelease];
		}
	}
...
{
	return _dividerView;//[[_dividerView retain] autorelease];
}
...
- (void)setDividerView:(MGSplitDividerView *)divider
{
	if (divider != _dividerView) {
		...
		_dividerView = divider;//[divider retain];
		_dividerView.splitViewController = self;
		_dividerView.backgroundColor = MG_DEFAULT_CORNER_COLOR;
		if ([self isShowingMaster]) {
			[self layoutSubviews];
...
{
	if (_cornerViews) {
		return _cornerViews;//[[_cornerViews retain] autorelease];
	}
